...
import { checkIfValidEndpoint, checkIfValidIP } from "../../utils/utils";
...
export function EndPointInput({ label, values, onChange, itemStyle = { bgColor: "vivo.pink", color: "gray.50" }, isDisabled = false }: EndPointInputProps) {
    ...
    function handleListItemRemove(item: string) {
        const newValues = listValues.filter((l: string) => (String(l) !== String(item)))
    ...
    function validateHostInput(newEndpoint: string) {

        const registeredValues = listValues;

        if (!newEndpoint) {
            setError({ error: true, message: "Insira um endpoint válido." });
            setIsError(true);

            return false;
        }
        if (!checkIfValidEndpoint(newEndpoint)) {
            setError({ error: true, message: "Insira um endpoint válido." });
            setIsError(true);
        ...
        if (registeredValues.includes(newEndpoint)) {
            setError({ error: true, message: "O endpoint já foi registrado." });
            setIsError(true);
